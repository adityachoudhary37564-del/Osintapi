...
@app.route('/api/search/file/<filename>', methods=['GET'])
def search_in_file(filename):
    ...
    if total_found == 0:
        return jsonify({
            'success': False,
            'message': f'No records found in {filename} for: {search_term}',
            'data': None
        }), 404
    
    return jsonify({
        'success': True,
        'message': f'Found {total_found} record(s) in {filename}',
        'total_results': total_found,
        'data': file_results
    }), 200
...
def home():
    return jsonify({
        ...
        'available_endpoints': [
            ...
            '/api/search/file/filename.xlsx?q=keyword',
            '/api/database-info'
        ]
    }), 200
